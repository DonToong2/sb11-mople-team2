...
-- 1. USERS
INSERT INTO users (id, created_at, email, password, name, profile_image_url,
...
-- 2. CONTENTS
INSERT INTO contents (id, created_at, updated_at, type, title, description,
...
FROM (SELECT gs,
      ...
      FROM generate_series(1, 10000) AS gs) AS data;
...
-- 4. PLAYLISTS
INSERT INTO playlists (id, created_at, updated_at, owner_id,
...
FROM (SELECT gs,
      ...
      FROM generate_series(1, 20000) AS gs) AS data
         JOIN (SELECT id,
                      ROW_NUMBER() OVER (ORDER BY id) AS user_no
               FROM users) AS u
              ON u.user_no = CASE -- 사용자 7000명은 플레이리스트를 만들지 않음
                                 WHEN gs <= 4000
                                     THEN 1 + floor(random() * 2000) -- 사용자 2,000명이 총 4000개의 플레이리스트를 분산 생성하도록 설정
                                 WHEN gs <= 9000
                                     THEN 2001 + floor(random() * 700) -- 사용자 700명은 총 5000개의 플레이리스트를 분산 생성하도록 설정
                                 WHEN gs <= 14000
                                     THEN 2701 + floor(random() * 250) -- 사용자 250명은 총 5000개의의 플레이리스트를 분산 생성하도록 설정
                                 ELSE
                                     2951 + floor(random() * 50) -- 사용자 50명은 6000개의 플레이리스트를 생성하도록 설정
                  END;
...
WITH numbered_contents AS ( -- (content_id, author_id) 복합 UNIQUE 제약 만족하도록 설정(콘텐츠 순번 부여)
    ...
    FROM contents),
     numbered_users AS ( -- (content_id, author_id) 복합 UNIQUE 제약 만족하도록 설정(사용자 순번 부여)
         ...
         FROM users)
INSERT
INTO reviews (id, created_at, content_id, author_id, text, rating)
SELECT gen_random_uuid(),
...
UPDATE contents c
SET rating_sum   = r.rating_sum,
    review_count = r.review_count
FROM (SELECT content_id,
             SUM(rating) AS rating_sum,
             COUNT(*)    AS review_count
      FROM reviews
      GROUP BY content_id) r
