  ...
  public @Nullable Message<?> preSend(Message<?> message, MessageChannel channel) {
    ...
    if (accessor != null && StompCommand.CONNECT.equals(accessor.getCommand())) {
      ...
      if (token == null) {
        ...
        throw new AuthException(AuthErrorCode.INVALID_TOKEN, Map.of("message", "웹소켓 CONNECT 요청에 인증 토큰이 누락되었습니다."));
      }
      ...
      try {
        ...
        userRepository.findById(userId).ifPresentOrElse(user -> {
          ...
          if (user.getSessionVersion() != tokenSessionVersion) {
            ...
            throw new AuthException(AuthErrorCode.EXPIRED_SESSION, Map.of("reason", "세션이 만료되었습니다."));
          }
          // 2. 관리자 등에 의해 잠금 처리된 계정인지 검증
          if (user.isLocked()) {
            log.warn("WebSocket 연결 거부: 비활성화(잠금) 상태의 계정 접근 - userId: {}", userId);
            throw new AuthException(AuthErrorCode.LOCKED_ACCOUNT, Map.of("reason", "접근이 제한된 계정입니다."));
          }
          ...
          throw new AuthException(AuthErrorCode.INVALID_TOKEN, Map.of("reason", "유효하지 않은 신원 정보입니다."));
        });

      } catch (ExpiredJwtException e) {
        log.warn("WebSocket 연결 실패: 만료된 JWT 토큰 사용 시도 - error: {}", e.getMessage());
        throw new AuthException(AuthErrorCode.INVALID_TOKEN, Map.of("authError", "EXPIRED_TOKEN"));

      } catch (MalformedJwtException | SignatureException e) {
        log.error("WebSocket 보안 경고: 변조되었거나 서명이 일치하지 않는 토큰 - error: {}", e.getMessage());
        throw new AuthException(AuthErrorCode.INVALID_TOKEN, Map.of("authError", "MALFORMED_TOKEN"));

      } catch (JwtException | IllegalArgumentException e) {
        log.warn("WebSocket 연결 실패: 기타 토큰 처리 예외 발생 - error: {}", e.getMessage());
        throw new AuthException(AuthErrorCode.INVALID_TOKEN, Map.of("authError", "INVALID_TOKEN"));
      }
    }
